...
//: `if` 문에서 조건은 반드시 부울 표현식이어야 합니다. 이것은 `if score { ... }`와 같은 코드는 잘못되었으며 암시적으로 0과 비교하지 않습니다.
//:
...
//: - Experiment:
//: `optionalName`을 `nil`로 변경하십시오. 어떤 인사말이 출력됩니까? `optionalName`이 `nil`인 경우 다른 인사말을 출력하도록 `else` 절을 추가해 보세요.
//:
...
//: - Experiment:
//: 변수를 하나 더 추가하여 어떤 종류(kind)가 가장 큰 숫자를 가지고 있는지와 그 숫자가 얼마인지 추적해보세요.
//:
