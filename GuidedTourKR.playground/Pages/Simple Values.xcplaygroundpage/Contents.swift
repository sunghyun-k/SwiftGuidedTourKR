...
//: - Experiment:
//: `Float` 자료형이 명시되어 있고, 값이 `4`인 상수를 생성해보세요.
//:
...
//: - Experiment:
//: 마지막 줄에서 `String`으로의 변환을 제거해 보십시오. 어떤 오류가 발생하나요?
//:
//: 문자열에 값을 포함하는 더 간단한 방법도 있습니다. 괄호안에 값을 쓰고 괄호 앞에 역슬래시(`\`)를 씁니다. 아래의 예시를 참조하십시오.
//:
let apples = 3
let oranges = 5
let appleSummary = "나는 \(apples)개의 사과를 가지고 있다."
let fruitSummary = "나는 \(apples + oranges)개의 과일을 가지고 있다."
...
//:
let quotation = """
나는 "나는 \(apples)개의 사과를 가지고 있다" 라고 말했다.
그리고 나는 "나는 \(apples + oranges)개의 과일을 가지고 있다." 라고 말했다.
"""
...
//: 유형 정보를 추론할 수 있는 경우, 변수에 새 값을 설정하거나 함수에 인자를 전달할 때, 빈 배열은 `[]`로 쓰고, 빈 딕셔너리를 `[:]`로 쓸 수 있습니다.
//:
shoppingList = []
occupations = [:]
